...
// .wrangler/tmp/bundle-ATuuv0/checked-fetch.js
...
// .wrangler/tmp/bundle-ATuuv0/strip-cf-connecting-ip-header.js
...
authRoutes.post("/signup", async (c) => {
  try {
    const body = await c.req.json();
    const { username, password, character_name, discord, alliance_id } = body;
    ...
    const result = await c.env.DB.prepare(
      `INSERT INTO users (username, password_hash, character_name, discord, alliance_id, role, is_approved)
       VALUES (?, ?, ?, ?, ?, ?, 0)`
    ).bind(username, passwordHash, character_name, discord || "", alliance_id || null, role).run();
...
attendanceRoutes.get("/", authMiddleware, async (c) => {
  try {
    ...
    const month = monthParam ? String(parseInt(monthParam)).padStart(2, "0") : String(now.getMonth() + 1).padStart(2, "0");
...
gameRoutes.post("/scores", authMiddleware, async (c) => {
  try {
    ...
    const validGames = ["reaction", "memory", "typing", "number"];
...
gameRoutes.get("/rankings", async (c) => {
  try {
    const games = ["reaction", "memory", "typing", "number"];
    const result = {};
...
// .wrangler/tmp/bundle-ATuuv0/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ATuuv0/middleware-loader.entry.ts
